...
DEFAULT_API = "https://maniquantai-api.vercel.app"
...
class BridgeApp:
    ...
    def connect(self):
        if self.running:
            return
        api = self.api.get().strip().rstrip("/")
        token = self.token.get().strip()
        if not api or not token:
            messagebox.showerror("Missing information", "Enter the ManiQuantAI API URL and bridge token.")
            return
        self.connect_btn.config(state="disabled", text="Connecting…")
        try:
            response = requests.get(f"{api}/api/mt5-bridge/jobs", params={"token": token}, timeout=10)
            response.raise_for_status()
        except Exception as exc:
            self.connect_btn.config(state="normal", text="Connect to MT5")
            self.set_status("Authentication failed", "#b42318")
            messagebox.showerror("Could not connect", f"The bridge token was rejected or the API is unreachable.\n\n{exc}")
            return
        self.running = True
        self.connect_btn.config(text="Bridge running")
        self.set_status("Authenticated — starting MT5…", "#b54708")
        self.worker = threading.Thread(target=self._run, args=(api, token), daemon=True)
        self.worker.start()

    def _run(self, api, token):
        try:
            run_bridge(api, token, 2.0, lambda text, account: self.set_status(text, "#067647" if text == "Online" or text == "Connected to MT5" else "#b54708", account))
        except Exception as exc:
            self.running = False
            self.root.after(0, lambda: self.connect_btn.config(state="normal", text="Connect to MT5"))
            self.set_status("Stopped", "#b42318")
