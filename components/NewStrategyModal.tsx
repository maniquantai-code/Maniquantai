...
export function NewStrategyModal({ onClose, onCreated }: Props) {
  ...
  async function handleCreate() {
    ...
    if (trimmed.length < 20) { setError("Add more detail — include symbol, timeframe, and entry condition."); return; }
    ...
    try {
      ...
      // Generate a clean name from the first 40 chars
      const name = trimmed.slice(0, 40).replace(/[·|]/g, "–").trim();

      const { data, error: dbErr } = await sb.from("strategies").insert({
        user_id: user.id,
        name,
        raw_strategy_text: trimmed,
        status: "created",
        spec: {},
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      }).select("strategy_id").single();

      if (dbErr) throw new Error(dbErr.message);
      onCreated(data.strategy_id);
      onClose();
    } catch (e: any) {
      setError(e?.message?.includes("duplicate") ? "You already have a strategy with this name." :
               e?.message?.includes("not logged") ? "Session expired — please refresh." :
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-lg rounded-2xl border border-border bg-bg-panel shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button onClick={onClose} className="rounded-lg p-1.5 text-text-muted hover:bg-bg-raised hover:text-text transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-text-muted leading-6">
            Describe your trading strategy in plain English. The agent team compiles it automatically
            and starts Research immediately.
          </p>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {EXAMPLES.map(ex => (
                <button key={ex} onClick={() => { setText(ex); setError(""); }}
        ...
        <div className="flex items-center justify-between border-t border-border px-6 py-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleCreate} disabled={loading || !text.trim()}
              ...
              {loading ? <><Loader2 size={14} className="animate-spin" /> Creating…</> : <><Zap size={14} /> Start pipeline</>}
            </button>
          </div>
        </div>
      </div>
