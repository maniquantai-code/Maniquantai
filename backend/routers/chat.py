...
async def mt5_connected(uid, token) -> bool:
    ...
    if not r.is_success:
        raise HTTPException(502, "Could not verify your broker connection")
    return bool(r.json())
...
async def chat(req: ChatRequest, background_tasks: BackgroundTasks, user=Depends(get_current_user)):
    ...
    if strategy_request(req.message):
        if not await mt5_connected(user["id"], token):
            ...
            action = "connect_mt5"
            response = "I’m ready to work on this strategy. First, connect your MetaTrader 5 account in Settings → Brokers so I can use your account’s market data. Once you’ve connected it, come back here and tell me **MT5 connected**. I’ll then ask you to confirm before research starts."
            ...
            response = "I’ve understood the strategy and your MetaTrader 5 connection is ready. Shall I start the Research Agent now? After research is complete, I’ll clearly tell you that research is done and move to the deterministic backtest."
        ...
        if not await mt5_connected(user["id"], token):
            ...
            state.pop("error", None)
            action = "confirm_research"
            response = "Great — your MetaTrader 5 account is connected. I’m ready to begin. **Shall I start the Research Agent?**"
            await save(req.strategy_id, user["id"], token, state, "ready")
        ...
        if not await mt5_connected(user["id"], token):
            ...
            state["agents"] = {**state.get("agents", {}), "research": "queued", "backtest": "queued", "indicator": "queued", "paper": "gated", "approval": "gated", "live": "gated"}
            state.pop("error", None)
            action = "strategy"
            response = "Confirmed. **Research Agent is starting now.** I’ll report when research is complete, then move directly to the deterministic backtest."
        ...
        if not await mt5_connected(user["id"], token):
            ...
            state["pending_confirmation"] = None
            action = "backtest"
            response = "Confirmed. **Deterministic Backtest Agent is running now.**"
