...
def _headers(token): return {"apikey":SUPABASE_ANON_KEY,"Authorization":f"Bearer {token}","Content-Type":"application/json","Prefer":"return=representation"}
async def _get(strategy_id,user_id,token):
 ...
 return r.json()[0]
async def _save(strategy_id,user_id,token,spec,status=None):
...
def _parse(text):
 s=text.upper(); symbol="BTC/USDT" if "BTC" in s else ("ETH/USDT" if "ETH" in s else None)
 m=re.search(r"EMA\s*\(?\s*(\d+)\s*\)?[^\n]{0,30}?EMA\s*\(?\s*(\d+)\s*\)?",s); periods=[int(m.group(1)),int(m.group(2))] if m else []
 ...
 return {"symbol":symbol,"timeframe":tf,"ema_fast":min(periods) if periods else None,"ema_slow":max(periods) if periods else None,"lookback_days":days,"timeframe_inferred":not ("15-MIN" in s or "15M" in s or "1-HOUR" in s or "1H" in s)}
async def _binance_klines(symbol,interval,days):
 end=int(datetime.now(timezone.utc).timestamp()*1000); start=end-days*86400000; rows=[]
 async with httpx.AsyncClient(timeout=20) as c:
  while start<end and len(rows)<20000:
   r=await c.get("https://api.binance.com/api/v3/klines",params={"symbol":symbol.replace("/",""),"interval":interval,"startTime":start,"endTime":end,"limit":1000}); r.raise_for_status(); batch=r.json()
   if not batch: break
   rows.extend((int(x[0]),float(x[4])) for x in batch); start=int(batch[-1][0])+1
...
def _ema(values,period):
 ...
 for i in range(period,len(values)):prev=values[i]*k+prev*(1-k);out[i]=prev
 return out
def _backtest(rows,fast,slow):
 closes=[x[1] for x in rows];ef=_ema(closes,fast);es=_ema(closes,slow);in_pos=False;entry=0.;trades=[]
 for i in range(1,len(closes)):
  if ef[i] is None or es[i] is None or ef[i-1] is None or es[i-1] is None:continue
  up=ef[i]>es[i] and ef[i-1]<=es[i-1];down=ef[i]<es[i] and ef[i-1]>=es[i-1]
  if not in_pos and up:in_pos=True;entry=closes[i]
  elif in_pos and down:trades.append(closes[i]/entry-1);in_pos=False
 if in_pos and entry:trades.append(closes[-1]/entry-1)
 wins=[p for p in trades if p>0];losses=[p for p in trades if p<=0];equity=peak=1.;max_dd=0.
 for p in trades:equity*=1+p;peak=max(peak,equity);max_dd=max(max_dd,(peak-equity)/peak)
 aw=sum(wins)/len(wins) if wins else 0;al=abs(sum(losses)/len(losses)) if losses else 0
 return {"trade_count":len(trades),"wins":len(wins),"losses":len(losses),"win_rate":round(len(wins)/len(trades)*100,2) if trades else 0,"net_return_pct":round((equity-1)*100,2),"max_drawdown_pct":round(max_dd*100,2),"avg_win_pct":round(aw*100,3),"avg_loss_pct":round(al*100,3),"win_loss_ratio":round(aw/al,3) if al else None,"data_bars":len(rows)}
async def run_pipeline(strategy_id,user_id,token):
 ...
 spec=_parse(strategy.get("raw_strategy_text") or "");state["research"]={"status":"complete","parsed_spec":spec,"completed_at":datetime.now(timezone.utc).isoformat()};state["agents"]["research"]="complete"
 if not spec["symbol"] or not spec["ema_fast"] or not spec["ema_slow"]:state["pipeline_stage"]="research_failed";state["error"]="Could not parse a supported crypto symbol and EMA pair.";await _save(strategy_id,user_id,token,state,"blocked");return
 ...
 try:rows=await _binance_klines(spec["symbol"],spec["timeframe"],spec["lookback_days"]);metrics=_backtest(rows,spec["ema_fast"],spec["ema_slow"])
...
async def start_pipeline(strategy_id,background_tasks:BackgroundTasks,user=Depends(get_current_user)):
