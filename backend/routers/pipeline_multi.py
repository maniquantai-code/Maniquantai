...
async def _accounts(uid: str, token: str):
    ...
    if not r.is_success:
        raise RuntimeError("Could not read trading account connections")
    return r.json()
...
async def _market_data(uid: str, sid: str, token: str, spec: dict):
    ...
    for account in ordered:
        try:
            ...
            elif account["connector_type"] == "broker_api":
                rows = await fetch_broker_api_candles(account, spec["symbol"], spec["timeframe"], spec["lookback_days"])
            else:
                continue
    ...
    try:
        return await yahoo(spec["symbol"], spec["timeframe"], spec["lookback_days"]), "Yahoo Finance"
    except Exception as exc:
        raise RuntimeError("No market-data source is available. " + "; ".join(errors + [f"Yahoo Finance: {str(exc)[:180]}"]))


def _runtime_from_compiled(compiled: dict) -> dict:
    runtime = compiled.get("runtime") or {}
    required = ("symbol", "timeframe", "lookback_days", "rsi_period", "rsi_entry_below", "rsi_exit_above", "bollinger_period", "bollinger_std", "risk_pct", "max_hold_hours", "stop_loss", "take_profit")
    missing = [k for k in required if k not in runtime]
    if missing:
        raise RuntimeError("Strategy compiler returned an incomplete runtime: " + ", ".join(missing))
    if runtime.get("symbol") == "unresolved" or runtime.get("timeframe") == "unresolved":
        raise RuntimeError("Strategy compiler could not resolve the instrument or timeframe")
    return runtime


async def _compile_and_store(strategy_id: str, user_id: str, token: str, strategy: dict, state: dict) -> dict:
    compiled = await compile_strategy(strategy.get("raw_strategy_text") or "")
    runtime = _runtime_from_compiled(compiled)
    state["strategy_spec"] = compiled
    state["parsed_strategy"] = runtime
    state["compiler"] = compiled.get("compiler", {})
    state["unresolved"] = compiled.get("unresolved", [])
    state["strategy_compiled_at"] = datetime.now(timezone.utc).isoformat()
    return runtime
...
async def run_research(strategy_id: str, user_id: str, token: str):
    try:
        ...
        spec = state.get("parsed_strategy")
        if not spec:
            spec = await _compile_and_store(strategy_id, user_id, token, strategy, state)
        ...
        _activity(state, "Research Agent started", f"Analysing {spec['symbol']} on {spec['timeframe']} using the selected market-data connector.", "running")
        await save(strategy_id, user_id, token, state, "research")
        rows, source = await _market_data(user_id, strategy_id, token, spec)
        minimum = max(int(spec["rsi_period"]), int(spec["bollinger_period"])) + 5
        if len(rows) < minimum:
            raise RuntimeError("Not enough market data for the requested indicators")
        state["data_source"] = source
        state["data_source_message"] = f"Market data source: {source}."
        ...
        _activity(state, "Research Agent complete", f"Verified {len(rows):,} bars from {source}. Deterministic backtest is ready for confirmation.")
...
async def run_backtest(strategy_id: str, user_id: str, token: str):
    try:
        strategy = await load(strategy_id, user_id, token)
        state = strategy.get("spec") or {}
        spec = state.get("parsed_strategy") or await _compile_and_store(strategy_id, user_id, token, strategy, state)
        ...
        _activity(state, "Deterministic Backtest Agent started", f"Testing {spec['symbol']} with deterministic rules; the LLM is not used to calculate performance.", "running")
...
async def start(strategy_id: str, background_tasks: BackgroundTasks, user=Depends(get_current_user)):
    ...
    state = strategy.get("spec") or {}
    try:
        if not state.get("strategy_spec"):
            await _compile_and_store(strategy_id, user["id"], token, strategy, state)
        state["pipeline_stage"] = "awaiting_research_confirmation"
        state["pending_confirmation"] = "research_start"
        state["agents"] = {"research": "gated", "backtest": "gated", "indicator": "gated", "paper": "gated", "approval": "gated", "live": "gated"}
        _activity(state, "Strategy compiled", "The user prompt was converted into a deterministic strategy specification. Research is waiting for user confirmation.")
        await save(strategy_id, user["id"], token, state, "ready")
        ...
        state["pipeline_stage"] = "strategy_compilation_failed"
        state["pending_confirmation"] = None
        state["error"] = str(exc)
        await save(strategy_id, user["id"], token, state, "failed")
        raise HTTPException(422, f"Strategy compilation failed: {str(exc)[:300]}")
...
async def confirm_research(strategy_id: str, background_tasks: BackgroundTasks, user=Depends(get_current_user)):
    ...
    state["pending_confirmation"] = None
    await save(strategy_id, user["id"], token, state, "research")
    background_tasks.add_task(run_research, strategy_id, user["id"], token)
    return {"action": "research", "content": "Confirmed. **Research Agent is starting now.**"}
