...
def _extract_json(text: str) -> dict[str, Any]:
    ...
    if not match: raise ValueError("Strategy compiler did not return JSON")
    value = json.loads(match.group(0))
    if not isinstance(value, dict): raise ValueError("Strategy compiler returned a non-object")
    return value
...
def _validate(spec: dict[str, Any], prompt: str) -> dict[str, Any]:
    ...
    if missing: raise ValueError(f"Compiled strategy is missing: {', '.join(missing)}")
    runtime = spec.get("runtime")
    if not isinstance(runtime, dict): raise ValueError("Compiled strategy runtime is invalid")
    ...
    if missing_runtime: raise ValueError(f"Compiled runtime is missing: {', '.join(missing_runtime)}")
    ...
    if not isinstance(spec.get("source"), dict): spec["source"] = {}
    spec["source"]["user_prompt"] = prompt
    return spec


async def compile_strategy(prompt: str) -> dict[str, Any]:
    if not prompt.strip(): raise ValueError("Strategy prompt cannot be empty")
    schema = _schema()
