...
class ExecutionPlanTests(unittest.TestCase):
    def test_ema_crossover_is_deterministic(self):
        ...
        signal = evaluate_plan(plan, bars_from_closes([1, 1, 1, 2, 3, 4]), 3)
        self.assertIsNotNone(signal)
        self.assertEqual(signal.side, "buy")
        self.assertEqual(signal.signal_key, "s1:4:buy")
    ...
    def test_existing_position_blocks_new_entry(self):
        ...
        plan = build_execution_plan(spec)
        self.assertIsNone(evaluate_plan(plan, bars_from_closes([1, 1, 1, 2, 3, 4]), 3, has_long=True))
